...
def plotall(arrays, **props):
    ...
    'plotfun'  (@plot_or_imagesc): function to use for plotting each element
    of data
    ...
    if nsubplots == len(arrays) and nsubplots > 40:
    ...
    return plot_pages(plot_funcs, not props['pub'])
...
def _initialize_subplots(narrays, props):
    ...
    if props['order'] == 'c':
        plot_order = plot_order.reshape(props['subplot'][::-1]).ravel('F')
