...
per bar.  Since the overwhelming majority (85\%) of our training data
consisted of bars that were 4 beats long, this resampling was not
often required.  Most of the remaining bars (14\%) were 3 beats in
length.  (FIXME: thierry, can you check these numbers, I'm making them
up based on the deletedfigure)
...
training sets ($1$K).
%
FIXME: Can we kill the ``non-random'' line.  It doesn't really add anything.
}}
...
perfect fifth (codewords 2 and 3) and fourth intervals (codewords 4
and 7), and a
major and minor triads (codewords 6 and 5, respectively).  Many of the remaining
codewords correspond to common transitions from one chord to another,
\eg a transitions between major triads in codes 8 and 10 ( Cmaj
$\rightarrow$ Fmaj)% assuming that chroma 0 is C) 
and the reverse transition in code 22 (\ie Fmaj $\rightarrow$ Cmaj).
