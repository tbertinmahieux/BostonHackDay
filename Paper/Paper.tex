...
alignment. Learning the same motifs with different offset
(of a few seconds, or audio frames) is redundant. We take advantage
of rythmic knowledge about the song to align patches with beats.
We can further group beats into bars, meaningful music segments.
The use of the Echo Nest API \cite{EchoNest} for that purpose is 
explained in Subsection \ref{ssec:beatpatch}. Patches built around
those temporal landmarks should capture music motifs more easily.
...
Practice showed us that they are reasonable, 
...
\ref{fig:data_sizes}). Regarding the covergence of the performance,
...
\end{center}
\caption{\small{
25 most frequently used codes in the codebook %in term of frequency of use
...
cluster centroids.  The results are shown \ref{tab:offset}. We find
