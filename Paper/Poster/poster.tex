...
%\usepackage{times} % recommended by Ron
...
\pbox{0.57\textwidth}{}{linewidth=2mm,framearc=0.3,linecolor=lightred,fillstyle=gradient,gradangle=0,gradbegin=white,gradend=whitepink,gradmidpoint=1.0,framesep=1em}{
%%% Title
\begin{center}
\begin{minipage}[c][10cm][c]{0.90\textwidth}
  \begin{center}
  ...
  {\sc \Huge in a Large Music Database}\\[10mm]
    {\Large Thierry Bertin-Mahieux $^\dagger$, Ron J. Weiss $^\ddagger$ and Daniel P. W. Ellis $^\dagger$\\[3.5mm]
     ...
     \large $\ddagger$ Music and Audio Research Laboratory, New York University \\
...
\begin{minipage}[c][7cm][c]{0.2\textwidth}
  \begin{center}
    ...
    \includegraphics[width=14cm,angle=0]{NYU_logo.eps}
  \end{center}
\end{minipage}
...
\item Availability of very large collections of music.
\item Can we infer anything about the underlying structure and common
features of pop music?
\item Our interest: tonal content of the music -- i.e. the
harmony and melody.
\item Beat-synchronous chromagrams: rich enough
to generate musically-relevant results,
abstract away instrumentation and stylistic details.
\end{list}
...
\myfig[0]{code_axis.ps}{.40}
\end{minipage}
\hspace{-3.5cm}
\begin{minipage}[l]{.4\columnwidth}
Typical %learned 
codeword spanning $2$ bars, each bar
considered to be $4$ beats. Codebook size was $200$.\\

A codewords can also be seen as the centroid of a cluster.
\end{minipage}
...
\item Entire database
represented by individual entries in this codebook.
\item Individual codewords consist of short beat-chroma patches of
...
vector (length $12$) for every music event (called ``segment'')
\item Also provides a
segmentation of the song into beats and bars.
...
(this is analogous to key normalization).
\end{list}
...
\myfig[0]{song_encoded.ps}{.68}
% figure caption
\begin{center}
\begin{minipage}[c]{.7\columnwidth}
\begin{small}
Example of an \textbf{encoded song} (Good Day Sunshine
...
\myfig[0]{codes_8bars.ps}{.68}
% figure caption
\begin{center}
\begin{minipage}[c]{.7\columnwidth}
\begin{small}
Longer \textbf{codewords spanning $\mathbf{8}$ bars}, randomly selected from
...
patches
\item Can be seen as online $k$-means
\item Initialized with $k$ random patches from the data
\item Although not optimal, scales linearly with
...
%  \begin{center}
%    \hspace{-1cm}
%    \includegraphics[width=0.52\textwidth]{conf_mat_per_artist.ps}
%  \end{center}
%  \vspace{-20pt}
%\end{wrapfigure}
...
\item Baseline: GMM - MFCC $59\%$, GMM - chromas $33\%$,
random $5\%$.
\item We get an accuracy of $\mathbf{23.4\%}$.
\end{list}

\myfig[0]{conf_mat_per_artist.ps}{.55}
...
\begin{list}{\labelitemi}{\leftmargin=2em \labelsep=1em}
\item Clustering based on the signal segmented in bars.
\item Codewords should contain bar information (e.g. a strong
first beat).
\item Our encoding find the right alignment $62\%$ of the time.
\end{list}
